...
class HtmlParser(private val context: Context) {
    ...
    fun getAppList(query: String): ArrayList<App> {
        ...
        val content = if (splitOne.isNotEmpty()) splitOne[1] else ""
        val splitTwo = content.split("class=\"listWidget\"")
        val listWidget = if (splitTwo.isNotEmpty()) splitTwo[1] else ""
        ...
        listWidget.split("class=\"appRow\"").forEachIndexed { index, s ->
            if (index > 0) {
                try {
                    ...
                    appList.add(App(title, dev, version, size, "$baseUrl$url", "$baseUrl$imageUrl"))
                } catch (e: Exception) {
    ...
    fun getAppScreenData(app: App): AppScreenData {
        ...
        return AppScreenData(description, versions, variants)
    }
}